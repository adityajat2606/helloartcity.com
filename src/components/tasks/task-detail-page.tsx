      ...
      <section className="mt-14">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {related.map((item) => {
            ...
            const description = (content as any).description || item.summary || "";
            return (
              <Link
                key={item.id}
                href={buildPostUrl(task, item.slug)}
                className="group rounded-[1.5rem] border border-stone-200 bg-white p-4 shadow-sm transition-all hover:shadow-md"
              >
                ...
                <div className="mt-4">
                  <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500">
                    {(content as any).category || item.tags?.[0] || category}
                  </span>
                  <h3 className="mt-2 text-lg font-semibold text-stone-950 line-clamp-2">{item.title}</h3>
                  {description ? (
                    <p className="mt-2 text-sm leading-relaxed text-stone-600 line-clamp-2">{description}</p>
                  ) : null}
                </div>
              </Link>
            );
...
export async function TaskDetailPage({ task, slug }: { task: TaskKey; slug: string }) {
  ...
  const detailContent = content.body || description;
  const descriptionHtml = !isArticle ? formatRichHtml(detailContent, "Details coming soon.") : "";
        ...
        <section className="mt-6 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <aside className="space-y-6">
            {images.length > 1 && (
              <div className="grid grid-cols-3 gap-3">
                {images.slice(1, 4).map((image, index) => (
                  <div key={`${image}-${index}`} className="relative aspect-square overflow-hidden rounded-[1rem]">
            ...
            <div className={`rounded-[2rem] p-6 ${experience.panelClass}`}>
              ...
              <h1 className="mt-4 text-2xl font-semibold tracking-[-0.04em] text-foreground sm:text-3xl">
                {post.title}
              </h1>
              ...
              {isArticle ? (
                <>
                  ...
                  {articleSummary ? <p className={`mt-4 text-sm leading-7 ${experience.mutedClass}`}>{articleSummary}</p> : null}
                  <RichContent html={articleHtml} className="mt-4 leading-7 prose-p:my-4 prose-h2:my-5 prose-h3:my-4" />
                </>
              ) : (
                <>
                  <RichContent html={descriptionHtml} className="mt-4" />
                  {content.highlights?.length ? (
                    <div className={`mt-4 rounded-[1.5rem] p-4 ${experience.softPanelClass}`}>
                      <p className="text-sm font-semibold text-foreground">Highlights</p>
                      <ul className={`mt-2 space-y-1 text-sm ${experience.mutedClass}`}>
            ...
            {isArticle ? (
              <div className={`rounded-[1.75rem] p-5 ${introCard}`}>
